...
export const WidgetKelasEdit = ({ kelasId }) => {
  ...
  useEffect(() => {
    const getKelasById = async () => {
      try {
        const kelas = await kelasApi.getKelasById(kelasId);
        kelas.tanggalMulai = parseISO(kelas.tanggalMulai);
        setKelas(kelas);
      } catch (error) {
        console.error(error);
      }
    ...
    <>
      ...
      >
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={() => updateKelasById(kelasId)}>
            Save
          </Button>
        </Modal.Footer>
      </Modal>
